        ...
        with open(link_file, 'rb') as fr:
            ...
            if len(script_link):
                for eachlink in script_link:
                    ...
                    # print linkTitleTmp
                    #将日期添加到lhc_titleName参数的最后面,增加变量文档生成日期
                    DocDate = "&DocDate="
                    DocDatePos = htmldata.find(eachlink)
                    DocDateStartPos = htmldata.find('<td>',DocDatePos)
                    DocDateEndPos = htmldata.find('</td>',DocDateStartPos)
                    DocDate = DocDate + htmldata[DocDateStartPos+4:DocDateEndPos].strip()
                    script_link_list.append(linkTmp + "?lhc_titleName=" + linkTitleTmp + DocDate)
