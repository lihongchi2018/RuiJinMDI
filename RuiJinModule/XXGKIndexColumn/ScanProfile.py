...
from RuiJinModule.XXGKIndexColumn.ScanPage import getAllLinkHTML
import win32api
'''
此文件为页面代码调用页面，采用线程处理方式，防止页面等待
通过线程调用ScanPage.py中的getAllLinkHTML函数
ScanPage.py引用的两个python文件为ScanPageHtml.py,ScanHttpLink.py，一般不需要修改参数，并引入MakeReport.py
ScanPageHtml.py文件主要实现扫描配置文件中的链接并生成链接页面HTML文件、提取页面中的链接、以及格式换链接为http://
ScanHttpLint.py文件主要实现检测上述已格式化的链接，检测否能访问并写入文件中
MakeReport.py根据用户对配置中栏目要求生成报告
'''

class ScanXXGKIndexColumn(QWidget):
    def __init__(self, UIBtn, UI, MainWindow):
        super(ScanXXGKIndexColumn, self).__init__()
        self.initUi(UIBtn, UI, MainWindow)

    def initUi(self, UIBtn, UI, MainWindow):
        UI.ScanTextEdit.append("Start XXGKIndexColumn Module ... ...")
        currPath="./RuiJinModule/XXGKIndexColumn"
        ...
        ProfilePath = "/Profile/bmgkxx_XXGKIndexColumn.profile"
        ...
        if os.path.exists(currPath + ProfilePath):
            ...
            # 3、扫描配置文件中的链接，并保存到bmlink_list,bmlink_title中,并将栏目更新天数保存到ColumnTimeLimit中
            with open(currPath + ProfilePath, 'r+') as fr:
                for line in fr.readlines():
                    linestr = line.strip()
                    if (linestr[0:1] == "#"):
                        if(linestr[0:2] == "##"):
                            UI.ScanTextEdit.append("此行为栏目更新时限设置：" +
                                                   linestr[2:linestr.index(":")] + " 更新时限为" + linestr[linestr.index(":")+1:] + "天")
            ...
            ScanThread = threading.Thread(target=getAllLinkHTML, args=(ColumnTimeLimit,currPath,
