...
'''
def ScanReport(ColumnTimeLimit,currPath,UI,bm_httpstatus_file,FileName_time_int,bmlink_title_list,FileName_time,bmlink_list):
    print("开始分析各单位检测状态文件")
    				...
    				<!DOCTYPE html>
    					...
    					<body>
    						<div style='margin:0 auto;text-align:center;margin-top:20px;'>
    							<div style='text-align:center;font-size:22px;'>部门信息公开首页栏目检测报告</div>
    ...
    bm_bg_str_head = bm_bg_str_head + """（首页更新日期以文件创建时间计算）		
    							...
    							<div style='text-align:center;'><table style='margin-top:10px;width:1000px;'>
    								<tr><th>序号</th><th>单位名称</th><th>更新日期</th><th>是否及时更新</th><th>首页不能访问链接数</th><th>详细报告</th></tr>
    					 """
    bm_bg_list.append(bm_bg_str_head)

    #各栏目检测html模板bm_bg_html_header，bm_bg_html_tailer
    bm_bg_html_header = """
    				<!DOCTYPE html>
    					<html>
    					<head>
    ...
    for status_file in bm_httpstatus_file:
        ...
        with open(status_file, 'rb') as fr:
            ...
            if date_str_int == 0:
                ...
                # raw_input("t")
                #此处要调用栏目时限配置文件
                if(bmlink_title_list[iTmp] in ColumnTimeLimit):
                    if delta.days < int(ColumnTimeLimit[bmlink_title_list[iTmp]]):
                        strUpdateStatus = "是（" + ColumnTimeLimit[bmlink_title_list[iTmp]] + "天内）"
                else:
                    #默认使用最短时限14天，两周内更新
                    if delta.days < 14:
                        strUpdateStatus = "是（14天内）"
   ...
   grader_father = os.path.abspath(os.path.dirname(pwd) + os.path.sep + "..")
    ...
    current_path = os.getcwd() + "\\RuiJinModule\\XXGKIndexColumn"
